...
export function useReadingContrast(scrollContainerRef: Ref<HTMLElement | null>): {
  ...
  function update() {
    ...
    sections.forEach((section) => {
       ...
       * Contact on phones sits low — mobile glow is bottom-centered; bias samples toward lower band + viewport
       * so --section-* matches per-node ink (.reading-word / .reading-icon).
       */
  ...
  /**
   * One coalesced rAF path applies section themes + per-word ink once per frame.
   * Eager `syncInkImmediate()` on every raw scroll event duplicated that work and walked
   * `.reading-word` / `.reading-icon` for each wheel/touch tick → main-thread jank.
   */
